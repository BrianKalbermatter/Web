...
var Stats = function () {
    ...
    // Función para recuperar el estado del juego desde localStorage
function cargarEstadoJuego() {
    const estadoGuardado = localStorage.getItem('estadoJuego');
    if (estadoGuardado) {
        return JSON.parse(estadoGuardado);
    } else {
        // Devolver un estado predeterminado si no se encuentra ninguno en localStorage
        return obtenerEstadoPredeterminado();
    }
}

// Ejemplo de un estado predeterminado del juego
function obtenerEstadoPredeterminado() {
    return {
        cuadricula: [[0, 1, 0], [0, 1, 0], [0, 1, 1]], // Aquí iría la representación de la cuadrícula
        piezaActual: 'T', // Aquí iría la pieza actual
        puntuacion: 100, // Puntuación del jugador
        nivel: 2 // Nivel actual del juego
        // Otros campos que necesites para representar el estado del juego
    };
}

// Ejemplo de cómo usar estas funciones
const estadoActual = {
    cuadricula: [[0, 1, 0], [0, 1, 0], [0, 1, 1]],
    piezaActual: 'T',
    puntuacion: 100,
    nivel: 2
};

//guardarEstadoJuego(estadoActual);

// Para cargar el estado del juego cuando el usuario regrese
const estadoCargado = cargarEstadoJuego();
console.log(estadoCargado);
...
function obtenerEstadoPredeterminado() {
    return {
        cuadricula: [1], // Aquí iría la representación de la cuadrícula
        piezaActual: 2, // Aquí iría la pieza actual
